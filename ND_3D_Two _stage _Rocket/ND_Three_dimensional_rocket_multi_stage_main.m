...
problem.n_length = n_length;
problem.n_velocity = n_velocity;
problem.n_mass = n_mass;
problem.n_thrust = n_thrust;
problem.n_time = n_time;

% Initial guess for decision variables
x0 = ND_Three_dimensional_initial_guess_new_vec(M,problem);


% Lower and Upper bounds for the variables
[lb, ub] = ND_Three_dimensional_lower_upper_bounds(M,problem);
...
load CB_vs_time.csv
time1 = CB_vs_time(1:20,1);
Cbx1 = CB_vs_time(1:20,2);
Cby1 = CB_vs_time(1:20,3);
Cbz1 = CB_vs_time(1:20,4);
Cbx1 = interp1(time1, Cbx1, t_1, 'spline');
Cby1 = interp1(time1, Cby1, t_1, 'spline');
Cbz1 = interp1(time1, Cbz1, t_1, 'spline');
time2 = CB_vs_time(20:202,1);
Cbx2 = CB_vs_time(20:202,2);
Cby2 = CB_vs_time(20:202,3);
Cbz2 = CB_vs_time(20:202,4);
Cbx2 = interp1(time2, Cbx2, t_2, 'spline');
Cby2 = interp1(time2, Cby2, t_2, 'spline');
Cbz2 = interp1(time2, Cbz2, t_2, 'spline');
...
200000);
    ...
    if strcmp(PS_method,'LGL')
       [x,fval,ef,output] = fmincon(@(x) ND_Three_dimensional_objective_func(x,M,problem),x0,A,b,Aeq,beq,lb,ub,@(x) ND_Three_dimensional_Nonlinear_func_LGL(x,M,D,problem),options);
    elseif strcmp(PS_method,'CGL')
       [x,fval,ef,output] = fmincon(@(x) ND_Three_dimensional_objective_func(x,M,problem),x0,A,b,Aeq,beq,lb,ub,@(x) ND_Three_dimensional_Nonlinear_func_CGL(x,M,D,problem),options);  
    end
...
% [c,ceq,dc,dceq]= ND_Three_dimensional_Nonlinear_func_LGL(x,M,D,problem);
...
Mach_1 = Vb_1./sqrt(Gamma*R*Static_Temp_1);
Mach_2 = Vb_2./sqrt(Gamma*R*Static_Temp_2);
...
collocation_points = t_1';
function_value = Cbx1;
Cbx1 = lagrange_interpolation_n(collocation_points, function_value, z_1);
% stage_2
collocation_points = t_2';
function_value = Cbx2;
Cbx2 = lagrange_interpolation_n(collocation_points, function_value, z_2);
% Multi_stage
Cbx = [Cbx1 Cbx2];

figure(10)
plot(z,Cbx,'g-',"LineWidth",2)
xlabel('Time [s]')
ylabel('Aerodynamic Body Coefficients')
grid on
hold on
% ylim([0 3]);
load CB_vs_time.csv
...
plot(ai1,ai2,'r--','LineWidth',2)
legend("PS Method","NPSOL");
title("Aerodynamic Body Coefficients Cbx w.r.t time")
% set(gca, 'FontSize', 40);
hold off
