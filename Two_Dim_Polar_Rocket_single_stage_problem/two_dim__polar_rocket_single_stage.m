...
20000);
    ...
    if strcmp(PS_method,'LGL')
       [x,fval,ef,output] = fmincon(@(x) two_dim_polar_rocket_single_stage_objective_func(x,N,m0),x0,A,b,Aeq,beq,lb,ub,@(x) two_dim_polar_single_stage_Nonlinear_func_LGL(x,N,D,problem),options);
    end
