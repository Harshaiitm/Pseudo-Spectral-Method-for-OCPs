...
addpath('C:\Users\Harshad\OneDrive\Desktop\Optimal Trajectory Transfer\goddard_rocket_multi_stage\PS_methods') % add the PS_method file directory
...
% 500000);
    ...
    if strcmp(PS_method,'LGL')
       [x,fval,ef,output] =opti_fmincon(@(x) multi_stage_objective_func_OPTI(x,N),x0,A,b,Aeq,beq,lb,ub,@(x) multi_stage_Nonlinear_func_LGL_OPTI(x,N,D,problem),opts);
    end
