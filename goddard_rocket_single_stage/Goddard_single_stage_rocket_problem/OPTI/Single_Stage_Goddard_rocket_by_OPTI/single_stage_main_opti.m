...
N = 0;     % Order of the polynomial
addpath('C:\Users\Harshad\OneDrive\Desktop\Optimal Trajectory Transfer\goddard_rocket_single_stage\PS_methods') % add the PS_method file directory
...
opts = optiset('solver','IPOPT','maxiter',5000,'maxfeval',20000,'tolrfun',1e-7,'tolafun',1e-7, ...
            'display','iter');
...
% 200000);
    ...
    elseif strcmp(PS_method,'LG') 
       [x,fval,ef,output] = opti_fmincon(@(x) single_stage_objective_func(x,N),x0,A,b,Aeq,beq,lb,ub,@(x) single_stage_Nonlinear_func_LG(x,N,D,problem),opts);
    elseif strcmp(PS_method,'LGR')
       [x,fval,ef,output] = opti_fmincon(@(x) single_stage_objective_func(x,N),x0,A,b,Aeq,beq,lb,ub,@(x) single_stage_Nonlinear_func_LGR(x,N,D,problem),opts);
    elseif strcmp(PS_method,'CGL')
       [x,fval,ef,output] = opti_fmincon(@(x) single_stage_objective_func(x,N),x0,A,b,Aeq,beq,lb,ub,@(x) single_stage_Nonlinear_func_CGL(x,N,D,problem),opts);
    end
